...
[[#ifndef ]b4_api_PREFIX[STYPE
    ...
    struct basic_symbol : Base
    {
      ...
      basic_symbol (const basic_symbol&) = delete;
      basic_symbol& operator= (const basic_symbol&) = delete;
    };
...
[b4_variant_if([], [[
  ...
  inline
  ]b4_parser_class_name[::by_type::by_type (by_type&& that)
    : type (that.type)
  {
    that.clear ();
  }
  ...
  inline
  ]b4_parser_class_name[::by_type&]
  b4_parser_class_name[::by_type::operator= (by_type&& that)
  {
    type = that.type;
    that.clear ();
    return *this;
